...
We assumed that we are storing our data in JSON format in a [file](../Application/Storage/repository.json) 
...
Under the resources folder, they saved a JSON file ([repository.json](../Application/Storage/repository.json)) that contains 
some data issued invoices from previous transactions.

> 💡 Tip: The total amount of each invoice is not included in this list. 

The Main class ([Program.cs](../Application/Program.cs)) initializes
...
1. Apply the above 2 patches to your code in [Invoice](../Application/Purchase/Invoice.cs) and 
[ReportGenerator](../Application/Report/ReportGenerator.cs) respectively 
1. Re-run the Main class ([Program.cs](../Application/Program.cs))
...
[Invoice](../Application/Purchase/Invoice.cs), reproduce the
