...
public class Elections
{
    ...
    public Dictionary<string, string> Results()
    {
        ...
        {
            nbVotes += _votesWithDistricts.Select(entry => entry.Value)
                .Select(districtVotes => districtVotes.Select(i => i)
                    .Sum())
                .Sum();
            ...
            {
                var index = _candidates.IndexOf(_officialCandidates[i]);
                nbValidVotes += _votesWithDistricts.Select(entry => entry.Value)
                    .Select(districtVotes => districtVotes[index])
                    .Sum();
            }
